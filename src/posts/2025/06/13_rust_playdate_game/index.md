...
Be sure to swap out the `todo!()` placeholder with `Ok(())` in the other 2 trait functions, otherwise Playdate will continue to crash when you try to launch the game. We'll add in the logic for these later.
